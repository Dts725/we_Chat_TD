function exchange(old) {
	...
	for (let j = 0; j < old.length; j++) {
		if (!old[j].day) {
			...
			//获取日期所在周数
			switch (new Date(old[j].day).getDay()) {
				case 0:
					old[j].day = old[j].day + " " + '周日'
					break;
				case 1:
					old[j].day = old[j].day + " " + '周一'
					break;
				case 2:
					old[j].day = old[j].day + " " + '周二'
					break;
				case 3:
					old[j].day = old[j].day + " " + '周三'
					break;
				case 4:
					old[j].day = old[j].day + " " + '周四'
					break;
				case 5:
					old[j].day = old[j].day + " " + '周五'
					break;
				case 6:
					old[j].day = old[j].day + " " + '周六'
					break;
			}
			//将moment字符串中的数字段截成数组
			let moment = old[j].moment.split(',')

			for (let i = 0; i < relationship.length; i++) {
				if (moment.length == 0) {
					if (moment[0] == relationship[i].id) {
						old[j].moment = relationship[i].time
					}
				} else {
					if (moment[0] == relationship[i].id) {
						old[j].moment = relationship[i].time.substr(0, 6)
					}
					if (moment[moment.length - 1] == relationship[i].id) {
						old[j].moment = old[j].moment + relationship[i].time.slice(6)
						break;
					}
				}
			}
